...
const stripCiAxisCodes = v => String(v == null ? "" : v)
  ...
  .replace(/\s{2,}/g, " ")
  .replace(/\s+([,.)\]])/g, "$1")
  // 조사 앞 공백만 정리한다. 단순 문자클래스(\s+[은는이가...])로 쓰면 "부각 가능성"→"부각가능성",
  // "판매 이후"→"판매이후"처럼 해당 글자로 "시작하는 단어"의 띄어쓰기까지 지운다.
  // → 뒤가 공백·문장끝·문장부호일 때(= 조사가 단독 토큰일 때)로 한정한다.
  .replace(/\s+(은|는|이|가|을|를|에|와|과|도)(?=\s|$|[,.)\]])/g, "$1")
  .trim();
...
async function loadInsights(env) {
  // 노출 기준: 당월 포함 최근 3개월 발행분만(filterRecentInsights). R2 피드·SEED 양쪽 동일 적용 →
  // 주간 갱신이 늦어져 피드가 노후해도 3개월 초과 카드는 메일에 실리지 않는다. 남는 카드가 6개 미만이면 있는 만큼만 노출.
  if (env.RESEARCH) {
    try {
      const o = await env.RESEARCH.get(INSIGHTS_KEY);
      if (o) { const a = await o.json(); const f = filterRecentInsights(a); if (f.length) return f; }
    } catch { /* 무시 → seed 폴백 */ }
  }
  return filterRecentInsights(INSIGHTS_SEED);
}
...
// 특정 기업과 무관한 거시·지정학 기사 판별.
// 기업명이 명시된 복합 기사는 기업 기사로 유지하고, 기업 태그가 없는 기사만 매크로 후보로 본다.
function isMacroNews(n) {
  const comps = (n.competitors || []).filter(Boolean);
  if (comps.length) return false;
  const text = [
    n.title, n.headline, n.summary, n.content, n.category,
...
function summaryContext(data) {
  ...
  const ciBoardTxt = (data.ciBoard || []).length
    ...
    : "없음";
  return `[지표 (hero·consume·cost 전용 — 시장 동향 newsSummary·news에는 사용 금지)]\n- ${L.join("\n- ")}\n\n[누적 신호(hero 전용)]\n${sigTxt}\n\n[CI 경쟁사 전략 추적 보드 — hero 전용, 시장 동향 newsSummary·news에는 사용 금지]\n${ciBoardTxt}\n\n[LG 전략축 관련 뉴스 — hero 전용, 시장 동향 newsSummary·news에는 사용 금지]\n${ciTxt}\n\n[가전 주요뉴스 — 시장 동향 newsSummary·news의 유일한 근거]\n${heads || "없음"}`;
}
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: [
            ...
            "- 출처 분리 원칙(최우선): 시장 동향 영역인 newsSummary와 news[]는 오직 [가전 주요뉴스]에 제공된 각 기사의 헤드라인·요약만 근거로 작성한다. [지표]·[누적 신호]·[CI 경쟁사 전략 추적 보드]·[LG 전략축 관련 뉴스] 및 기획리포트·아이디어·리서치 인사이트의 내용이나 표현을 newsSummary와 news[]에 섞지 않는다. 기사에 없는 회사 현황·전략·수치·사실을 보충하지 않는다.",
            ...
            "- newsSummary: [가전 주요뉴스] 기사 내용만 종합해 오늘 뉴스의 공통 핵심 의미를 한 문장(명사형 마무리)으로 쓴다. 건수·개별 제목 나열 금지. 기사들 사이에 공통 주제가 분명하지 않으면 억지로 하나의 흐름을 만들지 말고 가장 중요한 기사 흐름을 중심으로 쓴다. 전체 90자 이내. 뉴스가 없으면 빈 문자열.",
            ...
            "  · opportunity: 두 번째 화살표 — 해당 기사 내용에서 직접 확인되거나 직접 도출되는 유리한 시장 단서만 한 구(80자 이내)로 쓴다. 기획리포트나 외부 자료에서 당사 현황·강점·포지션을 가져오지 않는다. '기회:' 같은 라벨을 쓰지 말고 명사형으로 끝맺는다. 유리한 단서가 기사에 없으면 '당사에 유리하게 읽을 단서는 제한적'으로 적는다.",
            "  · threat: 세 번째 화살표 — 해당 기사 내용에서 직접 확인되거나 직접 도출되는 경쟁·시장·정책 압박만 한 구(80자 이내)로 쓴다. 기획리포트나 외부 자료의 판단을 가져오지 않는다. '위협:' 같은 라벨을 쓰지 말고 명사형으로 끝맺는다. 부담 단서가 기사에 없으면 '당장 부담으로 이어질 단서는 제한적'으로 적는다.",
            "  · 세 필드 모두 사실·방향 서술만 담는다. '검토 필요'·'대응해야'·'추진 여지' 等 실행 제안·액션 권고 금지 (실행 판단은 사람의 몫).",
            "  · 헤드라인·요약에 없는 사실·수치 창작 금지. content·opportunity·threat는 모두 채워 세 줄이 나오게 한다. 단, opportunity·threat 근거가 약하면 영향이 제한적이라는 문장으로 채운다.",
            ...
            "- [CI 경쟁사 전략 추적 보드 — 검증된 최근 동향]과 [LG 전략축 관련 뉴스 — 오늘 매칭·미검증 후보]는 hero에만 참고할 수 있다. newsSummary와 news[]에는 어떤 형태로도 반영하지 않는다.",
          ].join("\n"),

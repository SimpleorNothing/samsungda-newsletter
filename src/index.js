...
export function renderEmail(data, opts = {}) {
  ...
  const newsWhyRows = (w, fallbackContent = "") => {
    ...
    const row = (text, color) => `<div style="margin-top:5px;padding-left:13px;text-indent:-13px;font-size:13px;color:${T.text};line-height:1.7"><span style="display:inline-block;width:13px;text-indent:0;color:${color};font-weight:700">•</span>${esc(stripCiAxisCodes(text))}</div>`;
  ...
  const newsRows = data.news.length
    ? data.news.map((i, ni) => {
        ...
        ${newsWhyRows(sm.newsWhy[ni], stripCiAxisCodes(i.summary || ""))}
        </td>${thumb}
        </tr></table>`;
      }).join("")

...
- **근거 분리 원칙**: `newsSummary`와 뉴스별 `content`·`opportunity`·`threat`는 MI 기사 헤드라인·본문 요약만 근거로 작성한다. 기획리포트·아이디어·리서치 인사이트·CI 전략 보드·시장지표의 내용이나 표현은 시장 동향 요약에 사용하지 않는다. 기사에 없는 사실·수치·당사 현황을 외부 자료로 보충하지 않는다.

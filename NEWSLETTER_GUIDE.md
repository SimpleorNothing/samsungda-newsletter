...
- **환율 판정 규칙**: Yahoo 환율은 `USD 1 = 현지통화` 직접표기. 환율 숫자 하락은 해당 통화 강세, 상승은 해당 통화 약세로 역산한다. 생성 입력에는 환율 변화와 통화가치 변화를 함께 제공하고, AI 결과가 반대 방향을 서술하면 규칙 기반 원가 문장으로 자동 교체한다.
...
- **근거 분리 원칙**: `newsSummary`와 뉴스별 `content`·`opportunity`·`threat`는 MI 기사 헤드라인·본문 요약만 근거로 작성한다. 기획리포트·아이디어·리서치 인사이트·CI 전략 보드·시장지표의 내용이나 표현은 시장 동향 요약에 사용하지 않는다. 기사에 없는 사실·수치·당사 현황을 외부 자료로 보충하지 않는다. AI 요약 실패 시의 규칙 기반 대체 요약에도 동일하게 적용한다.
...
5. **기업별 인접 배치.** 동일 기업(계열 포함)을 다루는 기사가 여러 칸에 분산돼 있으면, 편집 시 서로 인접하도록 재배열한다.
6. **매크로/지정학 이슈는 최하단 배치.** 관세·환율·전쟁·정상회담 등 특정 기업과 무관한 거시·지정학 뉴스는 경쟁사·유관산업·신사업 기사보다 뒤, 호(號)의 맨 마지막 칸에 배치한다.

---

## 변경 이력
- **2026-08-07 12:07 (KST)** — 환율 강·약세 판정 및 자동 검증 보완 (#131) (`0f17423`)
- **2026-08-07 (KST)** — 환율 강·약세 역산 및 AI 결과 사후 검증 추가. 누적 신호에 환율 방향과 통화가치 방향을 함께 표기
- **2026-08-07 12:04 (KST)** — Merge pull request #126 from SimpleorNothing/feat/news-topic-diversity (`a6d343b`)
- **2026-08-06 09:34 (KST)** — 시장 동향 대체 요약도 기사 근거로 제한 (#129) (`c9a2fb8`)
- **2026-08-06 08:41 (KST)** — 시장 동향 요약을 기사 근거로 제한 (#128) (`da4a2fc`)
- **2026-08-04 23:22 (KST)** — 시장 동향 기사 배치 규칙 자동화 (#127) (`2e46194`)
- **2026-08-04 (KST)** — v1.3: 시장 동향에 기업별 인접 배치와 매크로·지정학 이슈 최하단 배치 규칙 추가
